...
const setBackground = (el) =>{
  const urlRegex =  /url(?:\(['"]?)(.*?)(?:['"]?\))/
  let url = el.getAttribute('lazy-background')
  if (!url) {
    // empty 1x1 PNG
    url = "data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAAAEAAAABCAQAAAC1HAwCAAAAC0lEQVR42mNkYAAAAAYAAjCB0C8AAAAASUVORK5CYII="
    }
  const currentStyle = el.style.backgroundImage
  const matches = urlRegex.exec(currentStyle)
  if (matches) {
    el.style.backgroundImage = currentStyle.replace(matches[1], url)
  } else {
    if (currentStyle) {

      el.style.backgroundImage = `${currentStyle}, url(${url})`
    } else {
    ...
    if(isWindowLoaded){
      entries.forEach(entry => {
        if(entry.isIntersecting){
          let el = entry.target;
          if(el.getAttribute('lazy-background') || el.getAttribute('lazy-background') === ''){
            setBackground(el);
          } else {
            setAttributes(el);
